    ...
    <AppLayout container>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 lg:py-12 space-y-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-4xl font-display font-bold tracking-tight text-foreground">Command Central</h1>
        ...
        <div className="grid gap-6 lg:grid-cols-7">
          <Card className="lg:col-span-4 border-none shadow-soft bg-zinc-50/50 dark:bg-zinc-900/50 overflow-hidden">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-lg font-display text-foreground">Content Velocity</CardTitle>
              ...
              <div className="h-[300px] w-full mt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: 'currentColor' }} dy={10} />
          ...
          <Card className="lg:col-span-3 border-zinc-200/60 dark:border-zinc-800/60 shadow-sm">
            <CardHeader>
              ...
              <div className="space-y-6">
                {posts.slice(0, 4).map((post) => (
                  <div key={post.id} className="flex items-center justify-between group cursor-pointer" onClick={() => navigate(`/editor/${post.id}`)}>
                    <div className="space-y-1">
                      <p className="text-sm font-semibold text-foreground group-hover:text-indigo-500 transition-colors">{post.title}</p>
                      <div className="flex items-center gap-2">
                        <Badge variant="secondary" className="text-[10px] px-1 py-0 h-4 uppercase">{post.status}</Badge>
                        <span className="text-[10px] text-muted-foreground">{format(new Date(post.updatedAt), 'MMM d, HH:mm')}</span>
                      </div>
                    </div>
                ...
                <div className="pt-4 border-t space-y-4">
                  ...
                  <div className="rounded-xl border border-indigo-100 dark:border-indigo-900/30 bg-indigo-50/30 dark:bg-indigo-900/10 p-4 relative overflow-hidden">
                    ...
                    <p className="text-sm font-display font-semibold mt-1 text-foreground">The Ethics of Generative CMS</p>
